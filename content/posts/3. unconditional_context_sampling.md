...
Diffusion models [4,8,9,11] have emerged as a flexible and expressive family of generative models, capable of producing high quality results in multiple domains, 
...
* [11] proposes a general method for conditional sampling from a jointly trained unconditional diffusion model $p(\textbf{x}=[\textbf{x}^{a},\textbf{x}^b])$ (where 
...
We train these models on a simple bimodal exponential dataset as shown below:
 <figure markdown="span">
...
Note that reconstruction guidance gives much more consistent trajectories unlike the replacement method which gives second-half trajectories that make sense on their
own but does not make sense in overall context.
...
to be robust to noisy inputs (as they have never seen noisy conditions during training). Indeed, we see that when we perturb the contexts (we use random bezier curves
as a form of perturbation) we get the following generations using the conditional model:

On the other hand, when using reconstruction guidance we get the following generations, which appears to be robust to these noisy inputs leading to good generations 
regardless:
...
1. Better approximations (other than reconstruction guidance), eg. Diffusion Posterior Sampling (as in [2]) or Twisted Diffusion Samplers [10] for more grounded
...
[10] Luhuan Wu, Brian L. Trippe, Christian A. Naesseth, David M. Blei, and John P. Cunningham.
Practical and Asymptotically Exact Conditional Sampling in Diffusion Models, 2023

[11] Yang Song, Jascha Sohl-Dickstein, Diederik P. Kingma, Abhishek Kumar, Stefano Ermon, and
